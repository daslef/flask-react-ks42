...
def change_task_status(name, task_id):
    ...
    with sqlite3.connect('app.db') as connection:
        ...
        global_id = user_tasks[task_id-1][0]
        current_status = user_tasks[task_id-1][6]
        cursor.execute('update tasks set status = ? where id = ?', [not current_status,
                                                                     global_id])
        connection.commit()
...
def delete_task(name, task_id):
    ...
    with sqlite3.connect('app.db') as connection:
        ...
        task_id = int(task_id.split('_')[-1])
        global_id = user_tasks[task_id-1][0]
        cursor.execute('delete from tasks where id = ?', [global_id])
        connection.commit()
